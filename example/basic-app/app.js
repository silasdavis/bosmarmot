...
// Connect to running burrow chain using the account address to identify our input account and return as an array
let chain = burrow.createInstance(chainURL, account.Address, {objectReturn: false})
...
app.post('/', (req, res) => param(req.body, 'value')
  ...
  .then(value => res.send({success: true, value: value}))
  .catch(err => res.send(handlerError(err))))
...
app.post('/:test', (req, res) => param(req.body, 'value')
  ...
  .then(([value, success]) => res.send({success: success, value: value}))
  .catch(err => res.send(handlerError(err))))
