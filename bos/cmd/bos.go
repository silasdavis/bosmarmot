...
func AddGlobalFlags() {
	...
	BosCmd.Flags().StringVarP(&do.Signer, "keys", "s", "localhost:10997", "IP:PORT of grpc keys daemon which jobs should use")
	...
	BosCmd.Flags().StringVarP(&do.BinPath, "bin-path", "b", "[dir]/bin", "path to the bin directory jobs should use when saving binaries after the compile process defaults to --dir + /bin")
	BosCmd.Flags().StringVarP(&do.ABIPath, "abi-path", "p", "[dir]/abi", "path to the abi directory jobs should use when saving ABIs after the compile process defaults to --dir + /abi")
